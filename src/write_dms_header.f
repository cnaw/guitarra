...
c
     ...
     &     gcount
      integer datamode, frame_divisor
      double precision bscale, bzero, dvalue
...
c
     ...
     &     template*50, eng_qual*8, program_id*(*)
      logical bkgdtarg
...
c
     ...
     &     det_sci_yangle, v3i_yang, wavstart, wavend, sporder
...
c
      if(verbose .ge.1) print *,'entered write_dms_header'
...
c      print *, cube_name, iunit, nx, ny, 
c     *     bitpix, naxis, naxes, pcount, gcount, filename,
c     *     title, pi_name, category, subcat, scicat, cont_id,
c     *     full_date,
c     *     date_obs, time_obs, date_end, time_end, obs_id,
c     *     visit_id, program_id, observtn, visit, obslabel,
c     *     visitgrp, seq_id, act_id, exposure_request, template
c      stop
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
c
      ...
      fw_filter = filter_id
c
! these should be defined in guitarra main, not here
...
c==================================================================
c     
      if(verbose.ge.2) print *,'write_tute_format: extnum', extnum
...
c     
     ...
     &        pcount,gcount,extend,status)
         ...
         if(verbose.ge.2) print *,'tute_keywords: ftphr : status',status
         if (status .gt. 0) then
            print *, 'simple,bitpix,local_naxis'
            call printerror(status)
         end if
         status =  0
c     
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ftpkys ', string
c     
         string = 'AZLAB'
...
c
         key = 'TIMEUNIT'
         string = 's'
         comment= 'Default unit applicable to all time values'
...
c     
         ...
         print *,'tute_keywords ftpkys FILENAME', filename
         print *,'tute_keywords ftpkys FILENAME', long_string
         if(verbose.ge.2) print *,'tute_keywords ftpkys FILENAME'
         comment= 'file name                              '
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ftpkys ', 
     &        long_string
c     
...
c
         key = 'SDP_VER'
         string = '2021_4'
         comment = 'Data processing software version number' 
...
!     
         key = 'PRD_VER'
         string = siaf_version ! SIAF file will have the number
         comment = 'S&OC PRD version number used in data processing'
         call ftpkys(iunit,key,string,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print 10, string
...
!     
         key = 'OSS_VER'
         string = '8.4'
         comment = 'Observatory Scheduling Software (OSS) version n'
         call ftpkys(iunit,key,trim(string),comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print 10, string
...
c
      hga_move = .FALSE.
      key      = 'HGA_MOVE'
      comment  = 'High Gain Antenna moved during data collection'
      call ftpkyl(iunit,key,hga_move, comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print 10, string
      end if
      status =  0
      if(verbose.ge.2) print *,'jwst_keywords: ftpkyl ', key
c
      key      = 'PWFSEET'
      comment  = 'Previous WFS exposure end time'
      value    = 0.0
      call ftpkye(iunit,key, value,-2, comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print 10, string
      end if
      status =  0
      if(verbose.ge.2) print *,'jwst_keywords: ftpkye ', key, value
      
c
      key      = 'NWFSEST'
      comment  = 'Next WFS exposure start time'
      value    = 0.0
      call ftpkye(iunit,key, value,-2, comment,status)
      if (status .gt. 0) then
         call printerror(status)
...
c     
         ...
         status =  0
c     
c         comment  = 'Science category assigned by TAC '
c         key = 'SCICAT'
c         call ftpkys(iunit,key, trim(scicat),comment,status)
...
c         status =  0
cc     
c         comment='continuation of previous program'
c         key = 'CONT_ID'
c         call ftpkyj(iunit,key,cont_id,comment,status)
c         if (status .gt. 0) then
c            call printerror(status)
c            print *, key
c         end if
c         status =  0
c         if(verbose.ge.2) print *,'tute_keywords: ftpkyj ', 
c     &        key,' ',cont_id
c     
c-------------------------------------------------------------------
c     
...
c
         string = 'TBD '
         comment= 'Quality of pointing information from EngDB'
         key = 'ENGQLPTG'
         call ftpkys(iunit,key,string,comment,status)
         if (status .gt. 0) then
            call printerror(status)
...
c     
         ...
         status =  0
c     
         comment= 'Total number of planned exposures in visit'
         key = 'NEXPOSUR'
...
c     
         ...
         status =  0
c     
         comment= 'Special commanding without SI configuration'
         key = 'EXP_ONLY'
         call ftpkyl(iunit,key,.FALSE.,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print 10, key
         end if
         status =  0
c     
         comment= 'Are the FGSes in a crowded field?'
         key = 'CROWDFLD'
         call ftpkyl(iunit,key,.FALSE.,comment,status)
         if (status .gt. 0) then
            call printerror(status)
...
c     
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',string
c     
...
c     
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',targ_ra
c     
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',targ_dec
c     
         comment='[deg] target RA uncertainty at mid-exposure'
         key = 'TARGURA'
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',targura
c     
         comment='[deg] target DEC uncertainty at mid-exposure'
         key = 'TARGUDEC'
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',targudec
c     
c     arc sec/year
c
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',mu_ra
c     
c     arc sec/year
c     
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',mu_dec
c
C     not contained in JWST version
c         comment='EPOCH of proper motion values'
c         key = 'MU_EPOCH'
c         call ftpkys(iunit,key,trim(mu_epoch),comment,status)
c         if (status .gt. 0) then
c            call printerror(status)
c            print 10, key
c         end if
c         status =  0
c         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',mu_epoch
c     
c     "degrees"
c     
         comment="Proposer's target RA"
         key = 'PROP_RA'
         ...
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ', key,' ',prop_ra
c     
c     "degrees" 
c     
         comment="Proposer's target Dec"
         key = 'PROP_DEC'
...
c
         ...
         call ftprec(iunit,card, status)
         status =  0
         card = '         Instrument configuration information     '
         call ftprec(iunit,card, status)
         status =  0
         card = '                                              '
         call ftprec(iunit,card, status)
...
c
         string = 'TBD'
         key    = 'OSF_FILE'
         comment = 'Observatory Status File name coverin'
         call ftpkys(iunit,key,string,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print 10, string
...
c     
         ...
         status =  0
c     
         comment='Number of detector amplifier outputs'
         call ftpkyj(iunit,'NOUTPUTS',n_outputs,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, 'NOUTPUTS'
         end if
...
c     
         key    = 'FRMDIVSR'
         frame_divisor = nframes
         comment='Divisor applied to frame-averaged groups'
         call ftpkyj(iunit,key,frame_divisor,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, 'NFRAMES'
         end if
...
c     
         ...
         status =  0
c     
         comment='Frames dropped at start of each RAMP'
...
c     
c     comment='Number of frames dropped between integrations'
!
         comment='Frames dropped at end of each RAMP'
         key = 'DRPFRMS3'
...
c     
         tsample = 10.d0
         comment='[us] Time  between samples'
...
c     
         key    = 'DATAMODE'
         datamode = 32 
         comment='  post-processing method used in FPAP'
         call ftpkyj(iunit,key,datamode,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, 'datamode'
         end if
...
c     
         key    = 'SCTARATE'
         dvalue  = 0.0d0
         comment='[ms/s] Spacecraft Clock Time Adjust RATE'
         call ftpkyd(iunit,key,dvalue,-6,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, 'SCTARATE'
         end if
...
c      status =  0
c
c     These are not contained in the JWST file, but I will keep them
c     as they are useful
c     
...
c     Time information
!
!     10. (Barycentric) time information
!
      card = '                                              '
      call ftprec(iunit,card, status)
      status =  0
...
!
!     I am skipping:
!     11. NIRCam Focus Adust Mechanism parameters
!     12. Guide Star Information
c-------------------------------------------------------------------
c     
c-------------------------------------------------------------------
...
c----------------------------------------------------------------------
c
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_KTC'
         card = '                                              '
         call ftprec(iunit,card, status)
         status =  0
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_RON'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_BKG'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_CR'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_DARK'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_DRKR'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_LAT'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_NLIN'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyl NOISELESS',
     &        ktc
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyj INC_FLAT'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyd KTC',
     &        ktc
         comment = 'KTC value (e-)'
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyd BIAS',
     &        bias_value
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyd BIASSIGM',
     &        voltage_sigma
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyd gain',
     &        gain
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyd readnoise',
     &        readnoise
...
c     
         if(verbose.ge.2) print *,'tute_keywords ftpkyd background',
     &        background
...
c     
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys BIASFILE'
...
c     
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys DRK_SLP'
...
c     
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys DRK_SIGM'
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys DRK_RAMP'
...
c     
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys WELL_DPT'
...
c     
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys GAIN_MAP'
...
c     
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys LIN_FILE'
...
c     
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys IPC_FILE'
...
c     
         ...
         if(verbose.ge.2) print *,'tute_keywords ftpkys FLATFILE'
...
c11111111111111111111111111111111111111111111111111111111111111111111111
c
c     Extension 1  data 4-D array EXTNAME == SCI XTENSION == IMAGE
c
      if(extnum .eq.1) then
c         
...
c         
c     verify that code does not crash
c         
...
c     already added by ftiimg:
c         key     = 'PCOUNT'
c         comment = 'number of parameters'
c         print *,'ftpkyj ', iunit, status
c         call ftpkyj(iunit,key,pcount,comment,status)
c         if (status .gt. 0) then
c            print *,'at ', key
c            call printerror(status)
c         end if
cc
c         key     = 'GCOUNT'
c         comment = 'number of groups'
c         print *,'ftpkyj ', iunit, status
c         call ftpkyj(iunit,key,gcount,comment,status)
c         if (status .gt. 0) then
c            print *,'at ', key
c            call printerror(status)
c         end if
c
         ...
         print *,'ftpkyd ', iunit, status
         call ftpkyd(iunit,key,bzero,-7,comment,status)
         if (status .gt. 0) then
            print *,'at ', key
            call printerror(status)
         end if
c
         ...
         comment = ' BSCALE * image + BZERO                   '
         print *,'ftpkyd ', iunit, status
         call ftpkyd(iunit,key,bscale,-7, comment,status)
         if (status .gt. 0) then
            print *,'at ', key
            call printerror(status)
         end if
...
c
c         comment = 'Extension value'
c         print *,'ftpkyj ', iunit, status
c         call ftpkyj(iunit,'EXTVER',extver,comment,status)
c         if (status .gt. 0) then
c            print *,'at EXTVER'
c            call printerror(status)
c         end if
c
         key     = 'MJD-BEG'
         dvalue  = expstart
         comment = '[d] exposure start time in MJD'
         call ftpkyd(iunit,key,dvalue,-12,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, key
            status = 0
...
c         
         key     = 'MJD-AVG'
         dvalue  = expstart
         comment = '[d] exposure mid-point in MJD'
         call ftpkyd(iunit,key,dvalue,-12,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, key
...
c         
         key     = 'MJD-END'
         dvalue  = expstart
         comment = '[d] exposure end time in MJD'
         call ftpkyd(iunit,key,dvalue,-12,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, key
...
c         
         key     = 'TDB-BEG'
         dvalue  = expstart
         comment = '[d] TDB time of exposure start in MJD'
         call ftpkyd(iunit,key,dvalue,-12,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, key
...
c         
         key     = 'TDB-MID'
         dvalue  = expstart
         comment = '[d] TDB time of exposure mid-point in MJD'
         call ftpkyd(iunit,key,dvalue,-12,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, key
...
c         
         key     = 'TDB-END'
         dvalue  = expstart
         comment = '[d] TDB time of exposure end in MJD'
         call ftpkyd(iunit,key,dvalue,-12,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, key
...
c         
         key     = 'XPOSURE'
         dvalue  =  -1.0
         comment = '[s] Effective exposure time '
         call ftpkyd(iunit,key,dvalue,-12,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, key
...
c
         key     = 'TELAPSE'
         dvalue  = -1.0
         comment = '[s] Total elapsed exposure time'
         call ftpkyd(iunit,key,dvalue,-12,comment,status)
         if (status .gt. 0) then
            call printerror(status)
            print *, key
...
c     
         ...
         status =  0
c
c     Telescope has these keywords here:
c     OBSGEO-X
c     OBSGEO-Y
c     OBSGEO-Z
c         
...
c
         ...
         status =  0
         if(verbose.ge.2) print *,'ftpky ',key, ' ',string
c
c----------------------------------------------------------------------
c     . Coordinate system
c     
         ...
         call ftprec(iunit,card, status)
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ftprec ', status
         card = '         Information about the coordinates in the file'
         call ftprec(iunit,card, status)
         status =  0
         if(verbose.ge.2) print *,'tute_keywords: ftprec ', status
         card = '                                              '
...
c-------------------------------------------------------------------
c
c     16. Spacecraft pointing information
c     (RA_V1, DEC_V1, PA_V3)
c
      ra_v1 = ra_ref
      dec_v1 = dec_ref
      status = 0
...
c
      key    = 'RA_V1'
      comment='[deg] RA of telescope V1 axis '
      call ftpkyd(iunit, key,ra_v1,-13,comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'dec_v1'
      end if
...
c
      key    = 'DEC_V1'
      comment='[deg] DEC of telescope V1 axis '
      call ftpkyd(iunit,key,dec_v1,-13,comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'dec_v1'
      end if
      status =  0
c
      comment = '[deg] Position angle of telescope V3 axis'
...
c      status =  0
c     
      comment='[arcsec] Telescope v2 coordinate of the reference'
      call ftpkyd(iunit,'V2_REF',v2_ref,-16,comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'v2_ref'
      end if
      status =  0
c     
      comment='[arcsec] Telescope v3 coordinate of the reference'
      call ftpkyd(iunit,'V3_REF',v3_ref,-16,comment,status)
      if (status .gt. 0) then
         call printerror(status)
         print *, 'v3_ref'
      end if
...
c
      ...
      status =  0
c
c     This keyword may also need to be placed in header 0
c      
