...
c
c     Marcia suggests adding flatfield here, prior to other instrumental
c     effects 2020.02.17. 
c
               if(include_flat .eq.1) then
                  call flat_multiply(n_image_x, n_image_y, subarray,
...
c
c     modified 2020-06-17. Coadd  now performs _no_ flatfielding
c     in spite of the value of "include_flat"
c               call coadd(include_flat)
               call coadd(include_flat, naxis1, naxis2)
            end if ! closes  "if(loop.le.nframe) then"
c     
