...
c     create the clonned 1-D image from 2-D templates
      subroutine prepare_clone_s(filename, z_sim, 
     &     nrc_index, sca_id, hst_level, nx, ny,
     &     x_scale, y_scale, sca_scale, one_d, nxny, npts,
     &     verbose)
c      
      implicit none
      integer ii, jj, nrc_level, ll, nx_zgal, ny_zgal,verbose,
     &     nrc_index, sca_id
c     
c     images
c
      character filename*(*), psf_conv*180
      double precision two_d, one_d
      double precision z_sim
!
      real cube, template, temp
      integer int_cube, naxes, hst_level, ntemplates, bitpix, extnum
      integer npts, nxny,  mmm, nnn,nnrc, ooo, level
      integer nx, ny, nz
c     
c     WCS
c
     ...
     *     ctype1*40,ctype2*40, ctype3*40
     ...
     &     cd1_1, cd1_2, cd2_1, cd2_2, cd3_3, equinox
      real (kind=8) ::  nrc, nrc_zp
      character nrc_name*6
c
...
!
!
      if(hst_level.eq.0) then
         call find_template_filter(redshift, z_sim,nrc(nrc_index),
     &        nrc_name(nrc_index), filter, wl_filter, nf,
     &        psf_conv, level, verbose)
      else
         level = hst_level
      end if
!
      if(verbose .gt.0) 
     &     print *,'prepare_clone_s : nx, ny, nz, level',
     &     nx, ny, nz, level
