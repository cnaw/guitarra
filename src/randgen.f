...
*
      ...
      INTEGER INIT
      INTEGER PID
      CHARACTER TEMPFILE*15, COMMAND*40
      DOUBLE PRECISION ZBQLIX(43),B,C
...
*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
C
C     In case of parallel programming make sure each process
C     is accessing a different file
C
       PID = GETPID()
       WRITE(TEMPFILE, 210) PID
 210   format('zbql_',i6.6,'.tmp')
       write(command,230) tempfile
 230   format(' date +%S%M%H%j > ',a15)
C       PRINT 240, command
 240   FORMAT(a40)
      CALL SYSTEM(command)
...
C 10    OPEN(FILNO,FILE='zbql1234.tmp',ERR=11)
 10    OPEN(FILNO,FILE=TEMPFILE,ERR=11)
       GOTO 12
 11    FILNO = FILNO + 1
