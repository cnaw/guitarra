...
print "reading pointings file : $pointings_file\n";
my ($pointings_ref, $pa_ref, $visit_ref) = get_apt_pointings($prefix, $pointings_file);
...
foreach my $key (sort(keys(%visit_content))) {
    ...
    if($debug != 0) {
	print "at line ", __LINE__," key : $key, visit_content: $visit_content{$key}\n";
    }
}

foreach my $key (sort(keys(%visits))){
    if($debug != 0) { print "at line: ",__LINE__," key:$key: visits:$visits{$key}\n";}
    my @junk = split(' ', $visits{$key});
    $visit_label_inv{$key}= $visits{$key};
    for(my $ii = 0 ; $ii <= $#junk ; $ii++) {
	if($debug != 0) {print "observation: $key visit: $junk[$ii]\n";}
	$visit_label{$junk[$ii]} = $key;
    }
}
if($debug != 0) {
    foreach my $key (sort(keys(%visit_label))){
	print "visit:$key observation: $visit_label{$key}\n";
    }
}
$debug = 0;
#die;
...
#				    print "at ",__LINE__," keys is $key, keyword :$keyword, junk :,$junk, targetname : $targetname, number: $number, visit_id{number}: $visit_id{$number}\n";
				}
				if($keyword eq 'Label') {
...
#						    print "at line ",__LINE__," visit_id_hash is $visit_id_hash, visit_id is $visit_id, visit_n is $visit_n, targetid is $targetid, value is $value\n";
						...
						if($counter7 == 0) {
						    $keyword  = $key6->nodeName;
						    $value    = $key6->textContent;
#						    print "at line ",__LINE__," $keyword $value\n";
...
#							print "at line ",__LINE__," $visit_id_hash : PrimaryDitherType is $primary_dither_type{$visit_id_hash}\n";
...
#										    print "at line ",__LINE__," keyword is $keyword\n";
										    if($keyword eq 'ShortFilter' || $keyword eq 'LongFilter' || 
										       $keyword eq 'ReadoutPattern' ||
										       $keyword eq 'Groups' || $keyword eq 'Integrations'){
											$value =~ s/\s/_/g;
...
#											print "at line ",__LINE__," $visit_setup{$visit_id_hash}\n";
										    }
										}
									    }
#########################################################################
...
#    print "at line ",__LINE__," reference is $reference ;  reference_orientation  is $reference_orientation\n";
    if($reference eq '') {
	print "at line ",__LINE__," variable reference is undefined: i.e. there is no orientation reference image\n\n";
#	print "This means that the orientation reference image is not defined\n";
#	print "at line ",__LINE__," variable reference_orientation is $reference_orientation\n";
#	$reference = ;
    } else {
...
#
	...
	print "at line ",__LINE__," visit_id_hash $visit_id_hash\n";
	$label = $visit_label{$visit_id_hash};
	if(exists($visit_orient{$visit_id_hash})) {
	    ...
	    if($reference_orientation ne '') {
		$visit_orient{$visit_id_hash} = $reference_orientation;
		$v3pa{$visit_id_hash}         = $reference_orientation;
	    }else{
...
#my ($pointings_ref, $pa_ref) = get_apt_pointings($pointings_file, $prefix);
#my (%pointings)              = %$pointings_ref;
#my (%pointings_pa)           = %$pa_ref;
#
...
#    print "\nkey of dithers_id is $key label is $visit_label{$key}\n";
...
foreach my $obs_folder (sort(keys(%visits))){
    ...
    print "at line: ",__LINE__," key:$obs_folder: visits:$visits{$obs_folder}\n";
    print "at line: ",__LINE__," observation: $obs_folder: visit: @array; obs_num: $obs_num\n";
    for (my $vm= 0 ; $vm <= $#array; $vm++){
	$visit_n = $array[$vm];
	$visit_n =~ s/\s//g;
# trying to make this compatible with existing code:
	$visit_number{$visit_n}= $obs_num;
	print "at line ",__LINE__," vm: $vm, visit_n $array[$vm], $visit_n $visit_content{$visit_n}\n";
#}
#foreach $visit_n (sort (keys (%visit_number))) {
	my $primary_dithers;
	my $primary_dither_type;
...
#
	...
	print "visit_content{$visit_n}:$visit_content{$visit_n} \n";
	print "at line ",__LINE__," visit_n: $visit_n observation_number: $visit_number{$visit_n} targetname: $targetname\n";
#    <STDIN>;
...
# needs a cleaner way to store these values
	...
	print "observation_number is $observation_number\n";
