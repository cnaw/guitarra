...
foreach my $obs_num (sort(keys(%observation_number))) {
    ...
    print "at line : ",__LINE__," observation number $obs_num label: $label visits: $nv dithers: $nd\n";
}
#die;
#print "pause\n";
if($testing == 1) {
...
#						    <STDIN>;
						...
						if($counter7 == 0) {
							...
							$value    = $key6->textContent;
							my @junk     = split('-',$value);
							$nirspec_setup{$visit_id_hash} = join(' ',$visit_id_hash,'IFU');
							$nirspec_setup{$observation_number} = 'IFU';
...
# Level 8
								...
								my $counter9 = @level9 ;
								if($testing == 1 || $debug == 1) {
								    print "at line : ",__LINE__," counter9 is $counter9 key8 is $key8\n";
								}
								$rr++;	
...
#											print "at line ",__LINE__," $visit_setup{$visit_id_hash} $observation_number\n";
										    }
										}
									    }
#########################################################################
...
#
    if($testing == 1) {
	print "at line : ",__LINE__," $target_id{$observation_number}, $target_number\n";
	print "at line : ",__LINE__,"  parameters for $obs_folder\n";
...
#	$visit_number{$visit_n}= $obs_num;
	if( ! exists($visit_number{$visit_n})) {
	    print "\nno visit_number for visit $visit_n at line ", __LINE__ ," -- using first visit's ($array[0]) setup data\n";
...
#
	...
	if($testing == 1) {print "at line : ",__LINE__," $visit_n, $visit_setup{$visit_n}\n";}

	my $target = $visit_n;
	my $pa_v3  = $visit_orient{$visit_n};
	my @telescope_moves = split('\#', $visit_content{$visit_n});
	my ($ra, $dec, $pa, $obs, $visit, $junk, $aperture);
	if($testing == 1) {
...
#	print "at line : ",__LINE__," primary dither $primary_dithers, subpixel_dither $subpixel_dither, targetname $targetname\n";
	my @lines = @$line_ref;
	for (my $ll = 0 ; $ll <= $#lines; $ll++) {
	    if($testing == 1) {
		print "at line : ",__LINE__," $lines[$ll]";
	    }
	    print OUT $lines[$ll];
	    print OUT1 $lines[$ll];
  	}
