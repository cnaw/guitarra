...
# Where are we going to put reduced data
    $reduced_dir       = '.';
#
    $cfg{'481'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCA1_17004_AZ.cfg';
    $cfg{'482'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCA2_17006_AZ.cfg';
    $cfg{'482'}  = '..ncdhas_dms/cal/SCAConfig/NRCA2_17006_AZ.cfg';
    $cfg{'483'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCA3_17012_AZ.cfg';
    $cfg{'484'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCA4_17048_SW_ISIMCV2.cfg';
    $cfg{'485'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCA5_17158_LW_ISIMCV2.cfg';
    $cfg{'486'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCB1_16991_SW_ISIMCV2.cfg';
    $cfg{'487'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCB2_17005_SW_ISIMCV2.cfg';
    $cfg{'488'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCB3_17011_SW_ISIMCV2.cfg';
    $cfg{'489'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCB4_17047_SW_ISIMCV2.cfg';
    $cfg{'490'}  = '/usr/local/nircamsuite/ncdhas_dms/cal/SCAConfig/NRCB5_17161_LW_ISIMCV2.cfg';
} 
...
if($host eq 'orange.as.arizona.edu' ||
   $host eq 'koa.as.arizona.edu' ||
   $host eq 'fenrir.as.arizona.edu' ||
   $host eq 'ema.as.arizona.edu') {
    $dhas = '/usr/local/nircamsuite/ncdhas_dms/ncdhas';
    $cal_path    = '/usr/local/nircamsuite/ncdhas_dms/cal/';
    $alt_flat_path = '/fenrirdata2/cnaw/commissioning/01063/skyflats/';
    
    if($no_flat == 1) {
	if($host eq 'fenrir.as.arizona.edu') {
	    $reduced_dir = '/fenrirdata2/cnaw/commissioning/01063/rates_no_flat/';
	}
    } else {
	$reduced_dir = '.';
    }
    # keep these for simulations
    if($simulation == 1) {
	...
	$cfg{'481'}  = $cal_path.'SCAConfig/NRCA1_17004_SW_FLIGHT.cfg';
	$cfg{'482'}  = $cal_path.'SCAConfig/NRCA2_17006_SW_FLIGHT.cfg';
	$cfg{'483'}  = $cal_path.'SCAConfig/NRCA3_17012_SW_FLIGHT.cfg';
	$cfg{'484'}  = $cal_path.'SCAConfig/NRCA4_17048_SW_FLIGHT.cfg';
	$cfg{'485'}  = $cal_path.'SCAConfig/NRCA5_17158_LW_FLIGHT.cfg';
	$cfg{'486'}  = $cal_path.'SCAConfig/NRCB1_16991_SW_FLIGHT.cfg';
	$cfg{'487'}  = $cal_path.'SCAConfig/NRCB2_17005_SW_FLIGHT.cfg';
	$cfg{'488'}  = $cal_path.'SCAConfig/NRCB3_17011_SW_FLIGHT.cfg';
	$cfg{'489'}  = $cal_path.'SCAConfig/NRCB4_17047_SW_FLIGHT.cfg';
	$cfg{'490'}  = $cal_path.'SCAConfig/NRCB5_17161_LW_FLIGHT.cfg';
    }
    #
    ...
    #
    $alt_flat{'481'} = $alt_flat_path.'skyflat_nrca1_';
    $alt_flat{'482'} = $alt_flat_path.'skyflat_nrca2_';
    $alt_flat{'483'} = $alt_flat_path.'skyflat_nrca3_';
    $alt_flat{'484'} = $alt_flat_path.'skyflat_nrca4_';
    $alt_flat{'485'} = $alt_flat_path.'skyflat_nrcalong_';
    $alt_flat{'486'} = $alt_flat_path.'skyflat_nrcb1_';
    $alt_flat{'487'} = $alt_flat_path.'skyflat_nrcb2_';
    $alt_flat{'488'} = $alt_flat_path.'skyflat_nrcb3_';
    $alt_flat{'489'} = $alt_flat_path.'skyflat_nrcb4_';
    $alt_flat{'490'} = $alt_flat_path.'skyflat_nrcblong_';
}
...
if($no_flat == 1) {
    $ncdhas_flags = '--ipc --wramp --outd '.$reduced_dir;
} else {
    $ncdhas_flags = '--ipc --cf --wramp';
}

if($reprocess == 1) {
    $ncdhas_flags = '--ow '.$ncdhas_flags;;
}
...
#
	...
	$slope_image  =~ s/.fits/.rate.fits/g;
	$wfs_image    =~ s/.fits/.wfs.fits/g;
	$diagnostic_image  =~ s/.fits/.dia.fits/g;
	$reduced_image    =~ s/.fits/.ramp.fits/g;
...
#	if($filter eq 'F150W2'){
#	$flat_image = $alt_flat{$sca_id}.$filter.'.fits';
#	}
...
#	    $command = join(' ',$dhas,$reduce_list[$i],$ncdhas_flags,$gain_flag);
	    if($no_flat == 1) {
		$command = join(' ',$dhas,'-f',$reduce_list[$i],
				'--ncpath','/usr/local/nircamsuite/ncdhas_dms',
				$ncdhas_flags, $cfg_flag);
	    } else {
		$command = join(' ',$dhas,'-f',$reduce_list[$i],
				'--ncpath','/usr/local/nircamsuite/ncdhas_dms',
				$ncdhas_flags, $cfg_flag, $flat_flag);
	    }
	} else {
